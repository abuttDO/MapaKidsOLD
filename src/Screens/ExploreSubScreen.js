...
export default class ExploreSubScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            showVideo: false,
                ...
                <div className={'col-12 sub-con'}>
                    <div id={'explore-sub'} className={'left-con row nomargin nopadding'}>
                        ...
                        <div className={this.state.mobile && this.state.vertical ? 'col-12 left-con-in' : 'col-6 left-con-in'}>
                            <div className={this.state.mobile && this.state.vertical ? 'in-btn-extra in-btn' : 'in-btn'}>
                                <button aria-label={'Ver en español'}
                                    className={'sub-btn es-btn'} onClick={() => this.showVideo(this.props.data.video_es)}/>
                                <button aria-label={'Ver en guaraní'}
                                    className={'sub-btn gu-btn'} onClick={() => this.showVideo(this.props.data.video_gu)}/>
                            </div>
                            <img className={this.state.mobile && this.state.vertical ? 'sub-info sub-info-extra' : 'sub-info'}
                                 alt={'gallery'}
                                 src={'/static/img/exploresub/' + this.props.data.image + '-sub.png'} />
                        </div>
