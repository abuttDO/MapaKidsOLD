  ...
  */
  var WordFindGame = function() {
    ...
    */
    ...
    // Game state
    var startSquare, selectedSquares = [], curOrientation, curWord = '', tout;
      ...
      create: function(words, puzzleEl, wordsEl, options) {
        ...
        if (window.navigator.msPointerEnabled) {
          ...
          if ('ontouchstart' in window) {
            $('.puzzleSquare').on('touchstart', function (e) {startTurn(e.target)});
            $('.puzzleSquare').on('touchmove', function (e) {
              var newItem = e.originalEvent.changedTouches[0];
              select(document.elementFromPoint(newItem.clientX, newItem.clientY))});
            $('.puzzleSquare').on('touchend', function (e) {endTurn(e.target)});
          }
        }
